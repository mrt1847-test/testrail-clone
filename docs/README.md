...
| [사용자 가이드](./user-guide/README.md) | 한국어 사용 절차와 촬영 기준·환경 제한 |
| [가이드 후속 작업](./USER_GUIDE_FOLLOW_UP_2026-09-24.md) | UG 항목의 범위·완료 기준 |
| [CI 예제](./CI_AND_COMPATIBILITY_EXAMPLES.md) | 기존 지원 API를 사용하는 운영 예제; 연동 작업 때만 읽기 |

`ux-evidence`에는 현재 제품 스펙의 before 화면과 로컬 UI-064~072 완료·UI-021 진행 상태를 입증하는 최신 증거를 유지한다. 새 작업의 검증 증거는 해당 ID로 기록하되 과거 캡처를 현재 완료 증거로 사용하지 않는다.
