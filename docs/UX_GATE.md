...
For UI-021, record one verdict per UX ID: actual/expected behavior, scenario/step, pass/fail/blocked/pending, viewport/role/build, evidence, and narrowly scoped remaining repair. UI-031/UI-051 storage and keyboard debt plus UI-051's pending review and instruction-load exception must be resolved or explicitly block the applicable gates. Preserve historical delivery checks; do not manufacture acceptance.

Confirmed violations of existing UX contracts use the user-authorized **UI-021 issue → automatic repair → revalidation** loop in [NEXT_ACTIONS](./NEXT_ACTIONS.md): record a stable issue ID and reproduction, deduplicate against existing UI work, add or link a scoped repair with acceptance conditions, leave UI-021 unchecked and move the first repair to Current. End the validation run after scheduling. A subsequent run implements exactly that repair; its evidence links back to the issue, and UI-021 rechecks after the repair queue completes. No new product code is implemented inside the validation run itself. New features/policies, ambiguous expected behavior, external authority/environment and required human review still need an explicit decision or readiness change. An unchanged blocker or the same evidence is not a reason to duplicate tickets or rerun the entire gate.
