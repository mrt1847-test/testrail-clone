...
- [ ] **TR-Core** P0 Case repository workbench shell: left suite/section tree, center compact case table, right selected-case detail pane, with section/case/filter/page state preserved in the URL. (Wave A–B + §6 API/UI: suite summary/grouped cases API, TR toolbar, right tree, Add Test Case sidebar)
...
- [x] **TR-Core** P0 Run execution workbench shell: persistent status count sidebar/rail, compact test table, and selected-test detail/result pane visible together without losing context. ([RUN_EXECUTION_VIEW_PARITY.md](./references/testrail/RUN_EXECUTION_VIEW_PARITY.md) Wave A–E; §8 체크리스트 — screenshot gate still recommended per TR-Core UX gate line above.)
- [x] **TR-Core** P0 Run execution speed actions: row status dropdown, Add Result, Pass & Next, next failed/blocked/untested, bulk results, and closed-run read-only behavior all usable from the execution workbench. (Same doc §8; inline status in grouped view, `RunExecutionToolbar`, keyboard shortcuts.)
