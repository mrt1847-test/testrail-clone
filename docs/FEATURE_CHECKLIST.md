...
- [ ] **TR-Core** P0 Run execution workbench shell: persistent status count sidebar/rail, compact test table, and selected-test detail/result pane visible together without losing context. (Ref: RUN_EXECUTION_VIEW_PARITY.md Wave A)
- [ ] **TR-Core** P0 Run execution speed actions: row status dropdown, Add Result, Pass & Next, next failed/blocked/untested, bulk results, and closed-run read-only behavior all usable from the execution workbench. (Ref: RUN_EXECUTION_VIEW_PARITY.md Wave B)
