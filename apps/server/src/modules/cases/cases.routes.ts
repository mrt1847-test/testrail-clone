...
export async function registerCasesRoutes(app: FastifyInstance, deps: { casesService: CasesService }) {
  app.get("/api/projects/:projectId/cases", async (req, reply) => {
    ...
    return reply.send(toJsonSafe(paged(deps.casesService.listCases(query), page, pageSize)));
  });

  app.get("/api/sections/:sectionId/cases", async (req, reply) => {
    const { sectionId } = sectionIdParamSchema.parse(req.params);
    const { page, pageSize } = paginationQuerySchema.parse(req.query ?? {});
    return reply.send(toJsonSafe(paged(deps.casesService.listCases({ sectionId }), page, pageSize)));
  });
  ...
  app.post("/api/sections/:sectionId/cases", async (req, reply) => {
    ...
    return reply.send(toJsonSafe(ok(deps.casesService.createCase(body))));
  });

  app.get("/api/cases/:caseId", async (req, reply) => {
    const { caseId } = caseIdParamSchema.parse(req.params);
    return reply.send(toJsonSafe(ok(deps.casesService.getCase(caseId))));
  });
  ...
  app.patch("/api/cases/:caseId", async (req, reply) => {
    ...
    return reply.send(toJsonSafe(ok(deps.casesService.updateCase(caseId, body))));
  });

  app.delete("/api/cases/:caseId", async (req, reply) => {
    requireProjectMutationRole(req);
    const { caseId } = caseIdParamSchema.parse(req.params);
    deps.casesService.deleteCase(caseId);
