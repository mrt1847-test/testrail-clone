...
export function ReportsOverviewPage() {
  ...
  const statusDistributionQuery = useQuery({
    ...
    queryFn: async (): Promise<StatusDistribution> => {
      const res = await apiFetch<Ok<StatusDistribution>>(`/api/projects/${projectId}/reports/status-distribution`);
      return res.data;
    },
  ...
  const failureTrendQuery = useQuery({
    ...
    queryFn: async (): Promise<FailureTrend> => {
      const res = await apiFetch<Ok<{ points: FailureTrend }>>(`/api/projects/${projectId}/reports/failure-trend`);
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {(runSummary ?? []).map((row) => (
              <li key={String(row.runId)} className="rounded border border-slate-200 px-3 py-2">
                <Link className="font-medium text-indigo-800 hover:underline" to={`/projects/${projectId}/runs/${row.runId}`}>
                  {row.name}
                </Link>
